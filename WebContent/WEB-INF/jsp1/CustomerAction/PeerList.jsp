...
<form action="${basePath}/background/CustomerAction.do?method=peerlistV2" method="post">
  <div class="am-collapse am-topbar-collapse" id="doc-topbar-collapse">
    <ul class="am-nav am-nav-pills am-topbar-nav">
      <li>注册时间:
		<input type="text" name="startTime"  class="Wdate" style="width:100px;height: 27px;" onclick="WdatePicker({dateFmt:'yyyy-MM-dd ',maxDate:'2100'})"/> -- <input name="endTime" type="text" class="Wdate" style="width:100px;height: 27px;"  value="${endTime }" onclick="WdatePicker({dateFmt:'yyyy-MM-dd ',minDate:'%y-%M-%d'})"/>
	 </li>
	...
	<option value="${peerGroup.name }">${peerGroup.name }</option>
      ...
      <li><input type="text"  name="obj" style="width:150px;"/></li>
      ...
      <div class="am-u-sm-12 am-u-md-6">
        <div class="am-btn-toolbar">
          <div class="am-btn-group am-btn-group-xs">
            ...
            <button type="button" class="am-btn am-btn-default" onclick="createNew('CustomerAction.do?method=addPeerUI');"> 导出excel</button>
            <button type="button" class="am-btn am-btn-default" onclick="createNew('CustomerAction.do?method=addPeerUI');"> 批量删除</button>
          </div>
        </div>
      </div>
    </div>
            ...
            <c:forEach items="${pageBean.recordList}" var="customer">
             ...
             <td>${customer.serialNumber}&nbsp;</td>
				...
				<td>	<c:forEach items="${customer.customerServicers}" var="cur">
					<label style="font-weight:bolder;" onclick="showName(${cur.id})">${cur.name }</label><br/>
				    </c:forEach></td>
				<td>	
