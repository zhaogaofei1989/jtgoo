    ...
    <div class="am-tabs-bd">
      <div class="am-tab-panel am-fade" id="tab2">
        <html:form styleClass="am-form" action="/background/VisaCountryAction" styleId="myform">
    		...
    		<html:hidden property="id" styleId="id" value="${param.id}"/>
    		<input type="hidden" name="swfupload" id="swfupload" value="true"/>
          <div class="am-g am-margin-top">
            <div class="am-u-sm-4 am-u-md-2 am-text-right">
            分类名称
            </div>
            <div class="am-u-sm-8 am-u-md-4">
           	 <html:select property="parentId" styleId="parentId" styleClass="SelectStyle">
                                <html:option value="">请选择分类</html:option>
                                <c:forEach items="${countryList}" var="country">
                                <c:choose>
                                <c:when test="${country.isparent}">
                            	  <html:option value="${country.id}">${country.name}</html:option>
                                </c:when>
                                <c:otherwise>
                                	<html:option disabled="true" value="${country.id}">${country.name}</html:option>
                                </c:otherwise>
                                </c:choose>
                                </c:forEach>
                            </html:select>
          ...
          <div class="am-g am-margin-top">
            ...
            <div class="am-u-sm-8 am-u-md-4">
            <html:text styleId="icon" property="icon" styleClass="am-input-sm"/>
            </div>
            <div class="am-hide-sm-only am-u-md-6"></div>
          </div>
        </html:form>
...
<div class="pad-10">
  ...
  <div class="am-margin">
    <button type="button" onclick="return saveInfo()" class="am-btn am-btn-primary am-btn-xs">提交保存</button>
...
$(function(){
	...
	onkeyup :false,// 是否在敲击键盘时验证 
			...
			if(id==='')
			{
				var name=$("#name").val();
			 $.ajax({  
                url: 'VisaCountryAction.do?method=checkName',  
                type: 'POST', 
				async:false, 
         		data : {
				name: name
