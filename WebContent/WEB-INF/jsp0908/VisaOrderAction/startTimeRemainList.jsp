 ...
 <!-- content start -->
	<div class="admin-content">
		<div class="am-cf am-padding">
      		<div class="am-fl am-cf"><strong class="am-text-primary am-text-lg">出发时间提醒 (${lingquVo.name})</strong></div>
    	</div>
    	<div class="am-g">
		...
		<div class="am-panel-hd" style="margin-left: 555px">
			<form action="${basePath}background/VisaOrderAction.do?method=startTimeRemainList" method="post">
	        ...
	        <table class="am-table am-table-striped am-table-hover table-main">
				...
				<tbody id="TableData" class="dataContainer">
					<c:forEach items="${pageBean.recordList}" var="visaOrder"  varStatus="status">
						<c:choose>
							<c:when test="${ visaOrder.days >0 and visaOrder.days <=5}">
								<tr class="odd" style="background-color: red;">
							</c:when>
							<c:when test="${ visaOrder.days >5  and visaOrder.days <=10 }">
								<tr class="odd" style="background-color: yellow;">
							</c:when>
							<c:otherwise>
								<tr class="odd">
							</c:otherwise>
						...
						<td>${ visaOrder.receivedDataTime3}</td>
...
<script type="text/javascript">
	...
	window.location.href="VisaOrderAction.do?method=exportExcel&label=003&id="+idArr+"&lingquId="+$("#lingquId").val();
	}
	else
	{
	alert("您还没有选中任何订单!")
